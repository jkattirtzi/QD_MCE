...
!  real(kind=8)::ma
  ...
  integer::nconf ! determind n.o configurations - MCE
  integer::pes! potential energy surface 1 or 2
...
!$********************************************************************************************************************!!
  SUBROUTINE READBSPAR(LINE1)
    ...
    do while (ierr==0)
       ...
       else if (LINE1=="PES") then
          backspace(27)
          read(27,*,iostat=ierr)LINE1,pes
          if(ierr.ne.0) then
             Print *,  "Error reading pes"
          endif
       else if (LINE1=="nconf") then
          backspace(27)
          read(27,*,iostat=ierr)LINE1,nconf
          if(ierr.ne.0) then
...
! function to do overlap of phis <phi|phi>
    Function ovlphiel(bf1,bf2)
      ...
      complex(kind=8)::aji
      Print *, 'ned to call mka but mka not here'
      STOP
      !call Mka (bf1,ai)
      !call Mka (bf2,aj)
