...
!$********************************************************************************************************************!!
  ...
  Function H_ord(bf1cz,bf2)
    ...
    else if (.not. (NameSys=='Morse' .OR. NameSys=='Harmonic' .OR. NameSys=='Free' .OR. NameSys=='MCE_sphub')) then
       Print *, 'WARNING, I havent understood name of system!!!!!!!!!!!'
    end if
    IF (NameSys=='MCE_sphub') then
      Print *, 'WARNING Havent assigned a Hord for MCE'
    END IF
  End Function H_ord
