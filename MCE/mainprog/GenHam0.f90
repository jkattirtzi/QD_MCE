...
!H_MCESpin is a subroutine that will get the hamiltonian matrix for the 2d spin boson model
     ...
     Subroutine H_MCESpin(bf1cz,bf2,HMceMat)
       ...
       !          print *, 'spdelta', spdelta, 'HMCEMAT', HmceMat(i,j)
       !          STOP
       !       else if ((i .eq. 1) .AND. (j .eq. 1)) then
       !          !Print *, 'in i=j=1'
                 HmceMat(1,2)=HordHarmB(bf1cz,bf2)+HordHarmC(bf1cz,bf2)+speps
       !          !Print *, 'B=', HordHarmB(bf1cz,bf2), 'C=', HordHarmC(bf1cz,bf2) ,'eps',speps
       !       else if ((i .eq. 2) .AND. (j .eq. 2)) then
                 HmceMat(2,1)=HordHarmB(bf1cz,bf2)-HordHarmC(bf1cz,bf2)-speps
       !       else  
...
!        integer,intent(in)::m
      ...
      Function DHehrNew(bf1cz,bf)
        ...
        !STOP
        DHMceMat(1,1)=DHordDz1cHarmB(bf1cz,bf)+DHarmCDz1c(bf1cz,bf)
        DHMceMat(1,2)=cmplx(0.0d0,0.0d0)
        DHMceMat(2,1)=cmplx(0.0d0,0.0d0)
        DHMceMat(2,2)=DHordDz1cHarmB(bf1cz,bf)-DHarmCDz1c(bf1cz,bf)
