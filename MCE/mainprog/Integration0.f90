...
!$********************************************************************************************************************!!
    ...
    Subroutine Derivs(t,bs,dbsdt)
      ...
      type(csbasisfn)::bf1cz,bf2, bf1, dbf1, dbf1cz
      ...
      complex(kind=8),dimension(:),allocatable::dconfdot
      complex(kind=8),dimension(nconf)::a,ac
      !complex(kind=8),dimension(:,:),allocatable::zdot
      ...
      do i=1,nbf
         ...
         call mka(bf1cz,ac)
         if (ndim .ne. 1) then
          print *, 'Ive simplified so ndim=1 and have assumed that to be true, but isnt here..stopping'
        !STOP
        end if
!         do j=1,ndim
            dbsdt(i)%z(1)=-1.0d0*Img*DHehrNew(bf1cz,bf1)     ! DHord takes in a j for a degree of freedom-have hbar=1
!         end do
      !Print *, 'in derivs  Z', 
      end do
      allocate(dconfdot(nconf))
      do i=1,nbf
        bf1=bs(i)
        call Ampdconfig(bf1,dconfdot)
        dbsdt(i)%allconf%d_config(1:nconf)=dconfdot(1:nconf)
      end do
!      print *, 'dconfd', dconfdot
!      STOP
      deallocate(dconfdot)
      !!!!!!!!!!!!!!!!!!!!!S derivs below:!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      do i=1,nbf
         ...
         call bfconz(bf1,bf1cz)
         call Mka(bf1,a)
         call H_MCESpin(bf1cz,bf1,HMceMat)
         dbf1=dbsdt(i)
         !call bfconz(dbf1,dbf1cz)
         dbf1cz%z(1)=-1.0d0*dbf1%z(1)
         call Mka(bf1cz,ac)
         !print *, 'ac', ac
         !print *, 'a', a
         !STOP
         do j=1,nconf
              dbsdt(i)%allconf%s_config(j)=(Img/2.0d0)*(dbf1%z(1)*bf1cz%z(1)-dbf1cz%z(1)*bf1%z(1))-HMceMat(j,j)*a(j)*ac(j)
              !dbsdt(i)%allconf%s_config(j)=Img*(Img*(dbf1%z(1)*bf1cz%z(1)-dbf1cz%z(1)*bf1%z(1))/2.0d0&
              !                             &-HMceMat(j,j))*a(j)

         end do  
      end do
! d_config derivs below
!      !!!!!!!!!!!!!!!!!!!!!D derivs below:!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!      allocate(ddot(nbf))
!      call Ampdmce (bs, ddot,dbsdt)
      dbsdt(1:nbf)%d=0.0d0!ddot(1:nbf)
!      deallocate(ddot)
