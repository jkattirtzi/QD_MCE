...
use BSET2, only:ndim, hbar, gam, m,w,nconf
...
real(kind=8)::g=0.0d0
real(kind=8)::spdelta,speps,spw,spalpha
contains
...
!$*******************************************************************************************************************!!
 ...
 !         end if
       if (NameSys=="Morse") then
          ...
          if(LINE1=="SpDelta") then
             backspace(30)
             read(30,*,iostat=ierr)LINE1, spdelta
             if (ierr .ne. 0) then
                Print *,'error in reading spdelta', ierr
             end if
             Print *, 'spdelta', spdelta
          end if
          if(LINE1=="SpEps") then
             backspace(30)
             read(30,*,iostat=ierr)LINE1, speps
             if (ierr .ne. 0) then
                Print *,'error in reading speps', ierr
             end if
          end if
          if(LINE1=="Spw") then
             backspace(30)
             read(30,*,iostat=ierr)LINE1, spw
             if (ierr .ne. 0) then
                Print *,'error in reading spw', ierr
             end if
             print *, 'just read spw', spw
          end if
          if(LINE1=="Spalpha") then
             backspace(30)
             read(30,*,iostat=ierr)LINE1, spalpha
             if (ierr .ne. 0) then
                Print *,'error in reading spalpha', ierr
             end if
          end if
       !   Print *, 'SpinBose parameters:'
       !   print *, 'SpDelta', spdelta
       !   print *, 'SpEps', speps
       !   print *, 'Spw', spw
      !    print *, 'Spalpha', spalpha 
       else
        Print *, 'ERROR STOPPING!!!!!!!!!!!!!!'
        Print *, 'havent understood system name and stopping'
        STOP
       end if
...
!$*******************************************************************************************************************!!
  ...
  End Function HordHarm
        ...
        Subroutine wom(w)
        Implicit None
        complex(kind=8),dimension(ndim)::w
        integer::i
        !print *, 'in wom, spw' , spw
        do i=1,ndim
        !w(i)=-spw*DLOG((1.0d0-DBLE(i)*(1.0d0-dexp(-spw*5.0d0/spw)))/ndim)
        w(i)=-1.0d0*spw*LOG((1.0d0-(DBLE(i)*(1.0d0-dexp(-5.0d0)))/DBLE(ndim)))
        !w(i)=-spw*DLOG(((1.0d0-DBLE(i))*(1.0d0-exp(-spw*5.0d0/spw)))/ndim)
        end do 
        End Subroutine

!HordHarmB is the hamiltonianB in the spin boson model

        Function HordHarmB(bf1cz,bf2)
        Implicit none
        type(csbasisfn),intent(in)::bf1cz,bf2
        complex(kind=8)::HordHarmB
        complex(kind=8),dimension(ndim)::w
        call wom(w)
        !HordHarmB=sum(w(1:ndim)*(bf2%z(1:ndim)*bf1cz%z(1:ndim)+0.5d0))
         HordHarmB=sum(w(1:ndim)*(bf2%z(1:ndim)*bf1cz%z(1:ndim)+0.5d0))
        End Function HordHarmB

  Function DHordDz1cHarmB(bf1cz,bf2,m)
    Implicit none
    ...
    integer,intent(in)::m
    complex(kind=8)::DHordDz1cHarmB
    integer::j
    complex(kind=8),dimension(ndim)::w
    call wom(w)
     print *, 'in dharmB'
    print *, 'wom', w(ndim)
    print *, 'bf2%z', bf2%z(ndim)
    !STOP
    DHordDz1cHarmB=(w(m))*(bf2%z(m))
 !   Print *, 'DHordDz1cHarmB', DHordDz1cHarmB
  End Function DHordDz1cHarmB

        Subroutine spinc(sc)
        Implicit None
        complex(kind=8),dimension(ndim)::sc,w
        integer::i
        call wom(w)
        do i=1,ndim
           sc(i)=w(i)*SQRT((spalpha*spw/DBLE(ndim))*(1.0d0-dexp(-spw*5.0d0/spw)))
        end do
  !      print *, 'sc', sc
  !      STOP
        End Subroutine

! HordHarmC is the hamiltonian for HarmC in the spin-boson model
        Function HordHarmC(bf1cz,bf2)
        Implicit none
        type(csbasisfn),intent(in)::bf1cz,bf2
        complex(kind=8)::HordHarmC
        complex(kind=8),dimension(ndim)::sc,w
        call spinc(sc)
        call wom(w)
        !HordHarmC=sum((sc(1:ndim)/SQRT(2.0d0*w(1:ndim)))*(bf2%z(1:ndim)+bf1cz%z(1:ndim)))
         HordHarmC=sum((sc(1:ndim)/SQRT(2.0d0*w(1:ndim)))*(bf2%z(1:ndim)+bf1cz%z(1:ndim)))
  !      print *, 'HordHarmC', HordHarmC
  !      STOP
        End Function HordHarmC

        Function DHarmCDz1c(bf1cz,bf2,j)
        Implicit None
        type(csbasisfn),intent(in)::bf1cz,bf2
        integer,intent(in)::j
        complex(kind=8)::DHarmCDz1c
        complex(kind=8),dimension(ndim)::sc,w
        call spinc(sc)
        call wom(w)
        !print *, 'sc', sc(ndim)
        !print *, 'w', w(ndim)
        !STOP
        DHarmCDz1c=(sc(j)/SQRT(2.0d0*w(j)))!*bf2%z(1))
        End Function DHarmCDz1c
